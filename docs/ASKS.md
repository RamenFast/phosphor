# ASKS — Ben's unique asks, tracked (filing-cabinet §6)

Seeded 2026-07-10 at audit-arc close; earlier asks live in HANDOFF.md's
release stories and git history. Append new unique asks; mark them done
with the commit/receipt that closed them.
...
| 2026-07-10 | UX round 1: silent local playback after spotify/mix switches; menu slows the scope; dismissal takes tries; cursor corruption + postcard dialog drags the window; more tooltips; "why do the same bugs reappear?" | DONE in-tree (BUGLOG #17-#19 + tooltip pass, 3 receipt rigs); recurrence answered in HANDOFF (shell decomp moved up the queue) |
| 2026-07-11 | "Did you install it on my local pc?" → installed 4.6.2+ux1 test build; then "yup ship" | SHIPPED — v4.7.0 tagged Latest (deb+rpm+source+SHA256SUMS), master pushed, installed==published (sha match), bench ALL PASS |
